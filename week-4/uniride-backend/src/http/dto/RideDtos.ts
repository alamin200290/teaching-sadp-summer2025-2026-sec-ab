...
export interface BookRideBody { riderId?: unknown; riderContact?: unknown; pickup?: unknown; dropoff?: unknown; seats?: unknown; }

export function parseBookRide(body: BookRideBody): RequestRideInput {
  const riderId = requireString(body.riderId, "riderId");
  const riderContact = typeof body.riderContact === "string" && body.riderContact.trim() ? body.riderContact : riderId;
...
function parseLatLng(value: unknown, field: string): Location {
  ...
  if (typeof v.lat !== "number" || v.lat < -90 || v.lat > 90) throw new ValidationError(`${field}.lat must be a valid latitude`);
  if (typeof v.lng !== "number" || v.lng < -180 || v.lng > 180) throw new ValidationError(`${field}.lng must be a valid longitude`);
  return new Location(v.lat, v.lng, typeof v.label === "string" ? v.label : undefined);
}
...
export function rideResponse(ride: Ride, driver: Driver, fare: Money, paymentRef: string) {
  return {
    ...
    fare: { amount: fare.amount, currency: fare.currency }, paymentRef,
  };
}
export function driverResponse(driver: Driver) {
  return {
    id: driver.id, name: driver.name, vehicle: driver.vehicle.describe(), seats: driver.vehicle.seats,
