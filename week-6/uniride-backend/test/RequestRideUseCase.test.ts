...
import { RideStatusPublisher } from "../src/rides/RideEvents";
...
import { NearestScorer } from "../src/matching/MatchScorer";
...
function wire(driverRepo: InMemoryDriverRepository, factory: DefaultNotificationChannelFactory) {
  ...
  const dispatcher = new NotificationDispatcher(factory, logger);
  const events = new RideStatusPublisher();
  events.subscribe(new NotifyingObserver(dispatcher));  // Observer sends the notifications
  return new RequestRideUseCase(
    new RideService(new InMemoryRideRepository(), clock, ids, logger),
    new MatchingEngine(driverRepo, config.matchRadiusKm, new NearestScorer()),
    new StandardFarePolicy(config),
    new PaymentService(new BkashGateway(), logger),
    events, driverRepo, logger);
}
test("end-to-end: matches a driver, prices, pays, notifies via observer, confirms", async () => {
