...
// SRP: the ride LIFECYCLE service. It creates and transitions rides and persists them.
// Every collaborator is an ABSTRACTION injected via the constructor (DIP) — which is
// exactly what makes this class unit-testable with in-memory fakes.
export class RideService {
  ...
  requestRide(riderId: string, pickup: Location, dropoff: Location, seats: number): Ride {
    ...
    this.logger.info("ride.requested", { rideId: ride.id, riderId });
    return ride;
  }

  assign(ride: Ride, driverId: string, fare: Money): void {
    ride.assignDriver(driverId, fare);
    this.rides.save(ride);
  ...
  start(ride: Ride): void {
    ride.start();
    this.rides.save(ride);
    this.logger.info("ride.started", { rideId: ride.id });
  }

  complete(ride: Ride): void {
    ride.complete();
    this.rides.save(ride);
    this.logger.info("ride.completed", { rideId: ride.id });
  }

  cancel(ride: Ride): void {
    ride.cancel();
    this.rides.save(ride);
    this.logger.warn("ride.cancelled", { rideId: ride.id });
  }
}
